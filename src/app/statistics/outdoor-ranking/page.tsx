...
import { OutdoorRankingTableWithModal } from "./OutdoorRankingTableWithModal";
...
async function fetchLeadersPage(
  ...
  const url = new URL(`${base}/api/outdoor-rank-point-leaders`);
...
export default async function OutdoorRankingPage({
  searchParams,
}: {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <p className="mb-4 text-sm text-[var(--chalk-muted)]">
        완등한 외벽문제의 랭크포인트 합계(문제당 1회) 순위 · 전체 {total}명
      </p>

      <form method="get" action="/statistics/outdoor-ranking" className="mb-6 flex flex-wrap items-center gap-2">
        <input type="hidden" name="size" value={pageSize} />
        <label htmlFor="outdoor-ranking-search" className="sr-only">회원명 검색</label>
        <input
          id="outdoor-ranking-search"
          type="search"
          name="q"
      ...
      {leaders.length === 0 ? (
        ...
        <>
          <OutdoorRankingTableWithModal leaders={leaders} />

          {totalPages > 1 && (
            <nav className="mt-6 flex flex-wrap items-center justify-center gap-2" aria-label="페이지 이동">
              {currentPage > 1 ? (
                <Link
                  href={`/statistics/outdoor-ranking${query(currentPage - 1)}`}
              ...
              {pageNumbers.map((n) => (
                <Link
                  key={n}
                  href={`/statistics/outdoor-ranking${query(n)}`}
